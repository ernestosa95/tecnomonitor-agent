...
| `elastic.checkdb_index` | string | `"ext_checkdb"` | Índice donde el pipeline publica un documento por base y por reinicio. El tipo de chequeo y la lista de bases de este camino se editan en `elk/ext_checkdb.conf` (bloque CONFIGURACIÓN de su `statement`), no acá |
